...
# Get legend
density_legend <- get_legend_fixed(seeding_density)
...
# Plot the predicted probabilities with confidence intervals
ggplot(sorted_data, aes(x = mean_fires_100, y = predicted_resprouting, color = woody_or_herb)) +
  geom_line(linewidth = 1) +
  geom_ribbon(aes(ymin = CI_low, ymax = CI_high, fill = woody_or_herb), alpha = 0.2, linetype = 0) +
  scale_x_log10() +
  ylab("Predicted Resprouting Probability") +
  xlab("Mean Fires") +
  theme_classic()
```
