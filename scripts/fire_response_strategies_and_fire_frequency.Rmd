...
Note: Results are still from the Poisson regression, rather than using just the mean fire frequency.
This includes the removal of 157 species which were unable to be fit with a valid set of coefficients when fitting the Poisson model.
Since Poisson lambda estimates are effectively (almost) the same as calculating the simple mean (see `mean_fires_poisson_method_for_macOS`
for comparison of the values generated with each method), we have kept the results from the Poisson regression.
...
# How many species in our particular dataset were scored as both perennial and annual?
df_woody_herb_only_life_history |>
  filter(str_detect(life_history, "perennial") & str_detect(life_history, "annual")) |>
  nrow() # 454

df_woody_herb_only_life_history |>
  filter(str_detect(life_history, "perennial") & str_detect(life_history, "annual")) |>
  pull(resprouting_binomial) |>
  table() # 68% of these are non-resprouters

df_woody_herb_only_life_history |>
  filter(str_detect(life_history, "perennial") & str_detect(life_history, "annual")) |>
  pull(seeding_binomial) |>
  table() # 88% of these are seeders
```
...
df_woody_herb_only_life_history |>
  filter(!is.na(annual_perennial)) |>
  group_by(woody_or_herb, annual_perennial) |>
  summarise(count = n()) # 683 annual herbs and 3544 perennial herbs
# 7 woody annuals - these are probably between woody and herbaceous and between annual and perennial
...
df_woody_herb_only_life_history |>
  ...
  summarise(
    num_resprout = sum(resprouting_binomial, na.rm = TRUE),
    num_resprout_data = sum(!is.na(resprouting_binomial)),
    prop_resprout = sum(resprouting_binomial, na.rm = TRUE) / sum(!is.na(resprouting_binomial)),
    num_seed = sum(seeding_binomial, na.rm = TRUE),
    num_seed_data = sum(!is.na(seeding_binomial)),
...
df_woody_herb_only_no_annuals <- df_woody_herb_only_life_history |>
  filter(annual_perennial != "annual") # Filters out NAs and annual "woody" species as well, 723 species

glm_resprouting_no_annuals <- glm(
  resprouting_binomial ~
    log10(mean_fires_100) + I(log10(mean_fires_100)^2) + log10(mean_fires_100):woody_or_herb +
    I(log10(mean_fires_100)^2):woody_or_herb + woody_or_herb,
...
# TODO Should I try adding life history as a predictor to the GLM?

summary(glm_resprouting_no_annuals)
confint(glm_resprouting_no_annuals)
sjPlot::tab_model(glm_resprouting_no_annuals)

ggeffects::ggpredict(glm_resprouting_no_annuals, c("mean_fires_100", "woody_or_herb")) %>%
  ggplot(aes(x, predicted, color = group)) +
  geom_line(linewidth = 1) +
  geom_ribbon(aes(ymin = conf.low, ymax = conf.high, fill = group), alpha = 0.2, linetype = 0) +
  scale_x_log10() +
...
# The relationship does not change when you remove annuals
```
...
confint(glm_resprouting_sp_only)
sjPlot::tab_model(glm_resprouting_sp_only)

summary(glm_seeding_woody)
summary(glm_seeding_woody_sp_only)
confint(glm_seeding_woody_sp_only)
sjPlot::tab_model(glm_seeding_woody_sp_only)
...
method_comparison |>
  ...
  scale_fill_manual(
    ...
    labels = c("Mean event rate", "Survival analysis")
  ) +
  labs(
    x = "Fire frequency (per century)",
    y = "Density",
