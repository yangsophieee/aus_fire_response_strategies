...
median_lma <-
  lma %>%
  group_by(taxon_name) %>%
  summarise(median_lma = median(leaf_mass_per_area)) %>%
  ungroup()
...
median_lma_new$family %>% unique() %>% length() # 186
x <- median_lma_new %>% group_by(family) %>% summarise(count = n())
hist(x$count, breaks = 100)

median_lma_summary <-
  median_lma_new %>%
  drop_na(median_lma, family) %>%
  group_by(family) %>%
  summarise(median_lma = median(median_lma)) %>%
  ungroup()
```
...
median_leaf_n <-
  leaf_n %>%
  group_by(taxon_name) %>%
  summarise(median_leaf_n = median(leaf_N_per_dry_mass)) %>%
  ungroup()
...
median_leaf_n_new$family %>% unique() %>% length() # 145
x <- median_leaf_n_new %>% group_by(family) %>% summarise(count = n())
hist(x$count, breaks = 100)

median_leaf_n_summary <-
  median_leaf_n_new %>%
  drop_na(median_leaf_n, family) %>%
  group_by(family) %>%
  summarise(median_leaf_n = median(median_leaf_n)) %>%
  ungroup()
```
