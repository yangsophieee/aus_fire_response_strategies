...
```{r}
make_phy_f <- function() {
  tt <- ape::read.tree("raw_data/v0.1-big-seed-plant-trees/ALLMB.tre")
...
lookup_table <-
  ...
  taxonlookup::lookup_table(by_species = TRUE, missing_action = "NA") %>%
...
resprouting_seeding_new$family %>% unique() %>% length() # 218 unique families
...
x <- mean_sla_new %>% group_by(family) %>% summarise(count = n())
hist(x$count, breaks = 100)
