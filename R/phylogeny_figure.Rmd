...
```{r}
# Update names with APCalign
lookup_table_phylogeny <- read_csv("data/lookup_table_phylogeny.csv")

# TODO Regenerate figures
# TODO Something wrong happened to the phylogeny
original_tree <- ape::read.tree("data/v0.1-big-seed-plant-trees/ALLMB.tre")
tt <- ape::read.tree("data/v0.1-big-seed-plant-trees/ALLMB.tre")

tt$tip.label <- if_else(is.na(lookup_table_phylogeny$accepted_name), tt$tip.label, lookup_table_phylogeny$accepted_name)
...
out_pruned <- ape::drop.tip(out, out$tip.label[!out$tip.label %in% resprouting_seeding_new$taxon_name])
# Align tree with APCalign #TODO

out_pruned <- ape::drop.tip(out, out$tip.label[!out$tip.label %in% resprouting_summary$family])
# Remove duplicates
out_pruned <- ape::keep.tip(out_pruned, unique(out_pruned$tip.label))

ggtree(out_pruned, ladderize = FALSE) +
  geom_tiplab() +
  xlim(0, 600)

ggsave("test_tree.png", width = 25, height = 15)
```
...
a <- taxonlookup::lookup_table(unique(tt$tip.label), by_species = TRUE, missing_action = "NA") |>
  select(-order, -group) # Get genus and familya <- rownames_to_column(a, "species")
a <- rownames_to_column(a, "species")
take_tenth <- function(x) x[10] # Not [1] because the first species of some families is taxonomically ambiguous
one_tip <- a |> group_by(family) %>% summarise(species = take_tenth(species))
out <- ape::drop.tip(tt, tt$tip.label[!tt$tip.label %in% one_tip$species])
out$tip.label <- paste(one_tip$family[match(out$tip.label, one_tip$species)], out$tip.label, sep = "_")
out$tip.label <- one_tip$family[match(out$tip.label, one_tip$species)]

out <- ape::keep.tip(out, out$tip.label[duplicated(out$tip.label)])
...
ggtree(new_tree, layout = "rectangular", size = 0.7) +
  ...
  geom_fruit(
    data = mean_lma_summary,
    geom = geom_text,
    mapping = aes(x = mean_lma, y = family, label = round(mean_lma, digits = 0)),
    colour = "black",
    offset = 0,
    pwidth = 0
  ) +
  ...
  geom_fruit(
    data = mean_leaf_n_summary,
    geom = geom_text,
    mapping = aes(x = mean_leaf_n, y = family, label = round(mean_leaf_n, digits = 1)),
    colour = "black",
    offset = 0,
    pwidth = 0
  ) +
...
ggsave("figures/phylogeny_figure_colour.png", width = 10, height = 9)
```