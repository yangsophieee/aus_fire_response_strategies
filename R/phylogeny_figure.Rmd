...
Try make plot again with gheatmap

```{r}
all_data <- resprouting_summary |>
  select(-data_count) |>
  left_join(seeding_summary, by = "family") |>
  left_join(mean_lma_summary, by = "family") |>
  left_join(mean_leaf_n_summary, by = "family") |>
  left_join(mean_fires_summary, by = "family") |>
  left_join(woodiness_summary, by = "family") |>
  relocate(mean_fires_100, .after = "family") |>
  as.data.frame()

rownames(all_data) <- all_data$family

all_data <- all_data |>
  select(-family)

base_tree <- ggtree(new_tree, layout = "rectangular", size = 0.7) +
  geom_tiplab(
    aes(subset = (!node %in% c(3, 7, 10, 14, 15, 19))),
    offset = 3,
    size = 4,
...
gheatmap(base_tree, all_data, offset = 20) +
