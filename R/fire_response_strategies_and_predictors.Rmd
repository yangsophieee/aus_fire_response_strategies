...
```{r}
resprouting_seeding_binomial <- readRDS("data/resprouting_seeding.rds")
```
...
```{r}
median_fris <- read_csv("data/median_fris.csv")
...
median_fires_per_50_years <- median_fris %>%
  mutate(across(
  c(med_w, med_w_lwr, med_w_upr, med_wo, med_wo_lwr, med_wo_upr),
  ~ 50 / .x
  ))
...
median_fris_wo <-
  ...
  select(
  taxon_name, med_wo, med_wo_lwr, med_wo_upr, num_unburnt_pixels, num_pixels, resprouting,
  post_fire_seeding, resprouter_or_seeder, data_on_both, taxonRank, resprouting_binomial
  )
```
...
mean_fris <- mean_fris %>% left_join(resprouting_seeding_binomial, by = c("taxon_name"))
```
...
mean_fris_clean <- mean_fris_no_errors %>% filter(upr_confint > 0)
```
