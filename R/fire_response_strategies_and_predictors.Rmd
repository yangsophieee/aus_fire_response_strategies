...
```{r}
median_fris <- read_csv("outputs/median_fris.csv")
...
median_fris_w <-
  ...
  select(
    ...
    resprouting_binomial, seeding_binomial, data_on_both, taxon_rank
  )
...
median_fris_wo <-
  ...
  select(
    taxon_name, med_wo, med_wo_lwr, med_wo_upr, num_unburnt_pixels, num_pixels, resprouting_binomial,
    seeding_binomial, data_on_both, taxon_rank
  )
```
...
median_fris_wo %>%
  ...
  mutate(n = n()) %>%
  mutate(label = paste0(seeding_binomial, "\nN = ", n)) %>%
  ggplot(aes(med_wo, factor(label), fill = factor(label))) +
  ggridges::geom_density_ridges() +
  scale_x_log10() +
  theme_classic() +
...
```{r}
mean_fris <- read_csv("outputs/mean_fri_df_with_glm.csv", guess_max = 10000)

mean_fris <- mean_fris %>% left_join(resprouting_seeding, by = c("taxon_name"))
```
...
```{r}
write_csv(mean_fris_no_errors, "outputs/final_df_predicted_fris_and_fire_response.csv")
mean_fris_no_errors <- read_csv("outputs/final_df_predicted_fris_and_fire_response.csv")
```
