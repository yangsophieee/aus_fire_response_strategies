...
joined_fire_data_inner <- readRDS("data/joined_fire_data_inner.rds")
```
...
calculate_mean_fri_with_glm <- function(
  ...
  if (length(model$error) < 1) {
    ...
    lwr <-
      sampling_period / (predict(model$result, type = "link")[[1]] + summary$coefficients[[2]] * 1.96)
