...
lma$leaf_mass_per_area <- as.numeric(lma$leaf_mass_per_area)
...
```{r}
fire_response_and_mean_fris <- read_csv("outputs/final_df_predicted_fris_and_fire_response.csv")
...
m1 <- lm(mean_lma ~ resprouting_binomial * woody_or_herb, data = leaf_traits_woody_herb_only)
summary(m1)
...
#        size = 4, fontface = "bold") +
  ...
  scale_y_log10() +
  labs(x = "Resprouting", y = bquote("LMA (" *g/m2 *")")) + #TODO switch from SLA to LMA
...
pm1 <-
  phylolm(mean_lma ~ resprouting_binomial * woody_or_herb,
      data = sorted_data,
      phy = pruned_tree,
      model = "BM")
summary(pm1)
...
```{r}
m3 <- lm(mean_lma ~ seeding_binomial * woody_or_herb, data = leaf_traits_woody_herb_only)
summary(m3)
...
#        size = 4, fontface = "bold") +
  ...
  scale_fill_manual(values = c("#915330", "#aedb65"),
            ...
            breaks = c("woody", "herb"),
            labels = c("Woody", "Herb")) +
  labs(x = "Post-fire Seeding", y = bquote("lma (" *mm^2/mg *")")) +
  theme_classic() +
  theme(panel.background = element_rect(colour = "black", fill = NA, size = 0.5),
...
```{r}
pm3 <-
  phylolm(mean_lma ~ seeding_binomial * woody_or_herb,
      data = sorted_data,
      phy = pruned_tree,
      model = "BM")
summary(pm3)
...
```{r}
grid::grid.draw(rbind(cbind(ggplotGrob(p1), ggplotGrob(p3)), cbind(ggplotGrob(p2), ggplotGrob(p4))))

final_plot <- rbind(cbind(ggplotGrob(p1), ggplotGrob(p3)), cbind(ggplotGrob(p2), ggplotGrob(p4)))
ggsave("outputs/leaf_traits.png", plot = final_plot, width = 8.5, height = 4.8)
```
...
lookup_table <-
  leaf_traits_woody_herb_only$taxon_name %>%
...
# Number of genera
leaf_traits_woody_herb_only$genus %>% unique() %>% length() # 1442

# Number of families
leaf_traits_woody_herb_only$family %>% unique() %>% length() # 215
```
...
```{r}
resprouting_data <- leaf_traits_woody_herb_only %>% drop_na(resprouting_binomial)
resprouting_data$genus %>% unique() %>% length() # 1436
resprouting_data$family %>% unique() %>% length() # 215

seeding_data <- leaf_traits_woody_herb_only %>% drop_na(seeding_binomial)
seeding_data$genus %>% unique() %>% length() # 734
seeding_data$family %>% unique() %>% length() # 149
```


Number of genera and families with fire frequency data

```{r}
# Filter out mean FRIs that have confidence intervals that include zero
data_clean <-
  leaf_traits_woody_herb_only %>%
  filter(upr_confint > 0) # Not sure this actually does what we want, confidence intervals could still include zero
data_clean$genus %>% unique() %>% length() # 1411
data_clean$family %>% unique() %>% length() # 211
```
...
```{r}
lma_data <- leaf_traits_woody_herb_only %>% drop_na(mean_lma)
lma_data$genus %>% unique() %>% length() # 720
lma_data$family %>% unique() %>% length() # 152

leaf_n_data <- leaf_traits_woody_herb_only %>% drop_na(mean_leaf_n)
leaf_n_data$genus %>% unique() %>% length() # 465
leaf_n_data$family %>% unique() %>% length() # 114
```


Number of genera and families with all data

```{r}
all_data <- data_clean %>% drop_na(mean_lma, mean_leaf_n, resprouting_binomial, seeding_binomial)
all_data$genus %>% unique() %>% length() # 268
