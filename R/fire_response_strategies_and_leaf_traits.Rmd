...
```{r}
fire_response_and_leaf_traits |> drop_na(mean_lma) |> nrow()
fire_response_and_leaf_traits |> drop_na(mean_lma) |> filter(woody_or_herb == "woody") |> nrow()
fire_response_and_leaf_traits |> drop_na(mean_lma) |> filter(woody_or_herb == "herb") |> nrow()

fire_response_and_leaf_traits |> drop_na(mean_leaf_n) |> nrow()
fire_response_and_leaf_traits |> drop_na(mean_leaf_n) |> filter(woody_or_herb == "woody") |> nrow()
fire_response_and_leaf_traits |> drop_na(mean_leaf_n) |> filter(woody_or_herb == "herb") |> nrow()
```
...
# Align with APCalign? #TODO
lookup_table_phylogeny <- read_csv("data/lookup_table_phylogeny.csv")

tree$tip.label <- if_else(
  is.na(lookup_table_phylogeny$accepted_name),
  tree$tip.label,
  lookup_table_phylogeny$accepted_name
)
...
# Remove duplicates
pruned_tree <- keep.tip(pruned_tree, unique(pruned_tree$tip.label))
