...
austraits <- read_rds("data/austraits-4.2.0.rds") # This is an updated rds file
```
...
lma <-
  ...
  pivot_wider(names_from = "trait_name", value_from = "value")
...
#        size = 4, fontface = "bold") +
  ...
  scale_y_log10() +
  labs(x = "Resprouting", y = bquote("lma (" *mm^2/mg *")")) +
...
ggsave("leaf_traits_v2.png", plot = final_plot,
...
# Number of genera
leaf_traits_woody_herb_only$genus %>% unique() %>% length() # 1114

# Number of families
leaf_traits_woody_herb_only$family %>% unique() %>% length() # 192
```
...
```{r}
leaf_traits_woody_herb_only %>% drop_na(resprouting_binomial) -> resprouting_data
resprouting_data$genus %>% unique() %>% length() # 1111
resprouting_data$family %>% unique() %>% length() # 192

leaf_traits_woody_herb_only %>% drop_na(seeding_binomial) -> seeding_data
seeding_data$genus %>% unique() %>% length() # 607
seeding_data$family %>% unique() %>% length() # 133
```
...
data_clean$genus %>% unique() %>% length() # 1088
data_clean$family %>% unique() %>% length() # 190
```
...
```{r}
leaf_traits_woody_herb_only %>% drop_na(mean_lma) -> lma_data
lma_data$genus %>% unique() %>% length() # 632
lma_data$family %>% unique() %>% length() # 142

leaf_traits_woody_herb_only %>% drop_na(mean_leaf_n) -> leaf_n_data
leaf_n_data$genus %>% unique() %>% length() # 395
leaf_n_data$family %>% unique() %>% length() # 107
```
...
```{r}
data_clean %>% drop_na(mean_lma, mean_leaf_n, resprouting_binomial, seeding_binomial) -> all_data
all_data$genus %>% unique() %>% length() # 246
all_data$family %>% unique() %>% length() # 78
```
