...
```{r}
lma_resp_model <- lm(
  mean_lma ~ resprouting_binomial * woody_or_herb,
  data = fire_response_and_leaf_traits)
summary(lma_resp_model)

lma_resp_emmeans <- emmeans::emmeans(
  lma_resp_model, specs = pairwise ~ resprouting_binomial:woody_or_herb)
lma_resp_emmeans
...
leaf_n_resp_model <- lm(
  mean_leaf_n ~ resprouting_binomial * woody_or_herb,
  data = fire_response_and_leaf_traits)
summary(leaf_n_resp_model)

leaf_n_resp_emmeans <- emmeans(leaf_n_resp_model, specs = pairwise ~ resprouting_binomial:woody_or_herb)
leaf_n_resp_emmeans$contrasts %>% confint()
```


```{r}
lma_seed_model <- lm(mean_lma ~ seeding_binomial * woody_or_herb, data = fire_response_and_leaf_traits)
summary(lma_seed_model)

lma_seed_emmeans <- emmeans(lma_seed_model, specs = pairwise ~ seeding_binomial:woody_or_herb)
lma_seed_emmeans$contrasts %>% confint()
```


```{r}
leaf_n_seed_model <- lm(mean_leaf_n ~ seeding_binomial * woody_or_herb, data = fire_response_and_leaf_traits)
summary(leaf_n_seed_model)

leaf_n_seed_emmeans <- emmeans(leaf_n_seed_model, specs = pairwise ~ seeding_binomial:woody_or_herb)
...
fire_response_and_leaf_traits <- fire_response_and_leaf_traits[fire_response_and_leaf_traits$taxon_name %in% tree$tip.label,]
...
pruned_tree <- drop.tip(tree, setdiff(tree$tip.label, fire_response_and_leaf_traits$taxon_name))
pruned_tree <- as.phylo(pruned_tree)

sorted_data <- fire_response_and_leaf_traits[order(match(fire_response_and_leaf_traits$taxon_name, pruned_tree$tip.label)),]
