...
Number of genera and families with SLA and leaf N data

```{r}
leaf_traits_woody_herb_only %>% drop_na(mean_sla) -> sla_data
sla_data$genus %>% unique() %>% length() # 632
sla_data$family %>% unique() %>% length() # 142
...
```{r}
data_clean %>% drop_na(mean_sla, mean_leaf_n, resprouting_binomial, seeding_binomial) -> all_data
