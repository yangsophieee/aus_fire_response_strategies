...
austraits <- read_rds("data/austraits-a8118973-20230629.rds") # Updated version of austraits as of 29/06/2023
```
...
resprouting_capacity <-
  ...
  distinct() %>% # Currently needed because duplicate trait values are not being collapsed
...
resprouting_capacity <-
  ...
  left_join(
    austraits$taxa %>%
      select(
        taxon_name, genus, family, taxon_rank, taxonomic_status, taxon_distribution
        ),
    by = "taxon_name")
```
...
resprouting_capacity_summary <-
  ...
  mutate(
    resprouting = if_else(
      ...
      case_when(
        resprout_tally < 0.3 ~ "does not resprout",
        (resprout_tally / (resprout_tally + does_not_resprout_tally) < 0.7) ~ "sometimes resprouts",
        (resprout_tally / (resprout_tally + does_not_resprout_tally) >= 0.7) ~ "mostly resprouts"),
      resprouting
      )
    )
```
...
post_fire_recruitment_summary <-
  ...
  mutate(
    post_fire_seeding = if_else(
      ...
      case_when(
        post_fire_recruitment_tally < 0.3 ~ "no post-fire seeding",
        (post_fire_recruitment_tally / (post_fire_recruitment_tally + no_post_fire_recruitment_tally) < 0.7) ~ "some post-fire seeding",
        (post_fire_recruitment_tally / (post_fire_recruitment_tally + no_post_fire_recruitment_tally) >= 0.7) ~ "robust post-fire seeding"),
      post_fire_seeding)
    )
```
...
post_fire_seeding <- post_fire_recruitment_summary %>%
  select(taxon_name, taxon_rank, post_fire_seeding)
```
...
```{r}
resprouting_taxa <- resprouting$taxon_name %>% unique() # 10735 taxa
seeding_taxa <- post_fire_seeding$taxon_name %>% unique() # 3594 taxa

intersecting_taxa <- intersect(resprouting_taxa, seeding_taxa)
# 3448 taxa intersect between post_fire_recruitment and resprouting_capacity
```
...
# Align taxon_rank values with gbif taxon_rank values
resprouting_seeding$taxon_rank <-
  resprouting_seeding$taxon_rank %>%
...
# 4503 herbs, 6229 woody, 63 semi-woody, 45 unclear, 11 NA
```
...
```{r}
#gbif <- read_csv("data/filtered_aus_obs.csv", guess_max = 13607514)
#gbif <- read.csv("data/filtered_aus_obs.csv", fileEncoding = "ISO-8859-1", skipNul = TRUE)
gbif <- read.csv("data/filtered_aus_obs.csv", skipNul = TRUE)
#gbif <- fread("data/filtered_aus_obs.csv", quote = "")
```

## Try use ausflora to align GBIF names with APC names

```{r}
#remotes::install_github("yangsophieee/ausflora", ref = "sophie_debugging") # Version with fixed bug
remotes::install_github("traitecoevo/APCalign")
detach("package:APCalign", unload = TRUE)
library(APCalign)
```
...
gbif <-
  gbif %>%
  left_join(taxonomic_updates, by = c("verbatimscientificname" = "original_name"))
```
...
joined_fire_data <-
  resprouting_seeding %>%
  inner_join(gbif, by = c("taxon_name" = "canonicalName"))

joined_fire_data %>% write_rds("data/joined_fire_data.rds")
joined_fire_data <- read_rds("data/joined_fire_data.rds")
...
```{r}
calc_taxa_proportions <-

  function(data, value, na.rm = TRUE) {
    browser()
...
make_taxa_proportion_plot <- function(df, variable, pixel_size = 1) {
  ...
  fire_data <- df
  values_in_var <-
    fire_data[,variable] %>% pull() %>% unique() %>% discard(is.na)
...
# terra::rasterize doesn't work the same way as raster::rasterize, it only summarises the geometries in the cell, not the data values associated with the geometries
...
```{r}
list_of_files <-
  list.files("data/MODIS_burn_data_australia/Win20",
             recursive = TRUE,
             full.names = TRUE) %>%
...
```{r}
init_raster <- raster("data/MODIS_burn_data_australia/Win20/2000/MCD64monthly.A2000306.Win20.006.burndate.tif")
# Resolution = 0.004
...
writeRaster(num_fires_raster, "data/num_fires_map", format = "GTiff", overwrite = TRUE)
```


```{r}
num_fires <- raster("data/num_fires_map.tif")
num_fires <-
  num_fires %>%
  as.data.frame(xy = TRUE)
...
num_fires_plot <-
  num_fires %>%
  filter(!is.na(num_fires_map)) %>%
  ggplot(aes(x = x, y = y)) +
  geom_raster(aes(fill = num_fires_map), interpolate = FALSE) +
  geom_polygon(data = map_australia,
               mapping = aes(x = long, y = lat, group = group),
               fill = NA,
               colour = "#1a1a1a",
...
ggarrange(p1, p2, num_fires_plot, ncol = 3, nrow = 1, labels = c("a) Proportion of Resprouting Taxa", "", "b) Number of Fires"))
ggarrange(ggarrange(p1, p2, ncol = 2, nrow = 1, common.legend = TRUE, legend = "right"), num_fires_plot,
