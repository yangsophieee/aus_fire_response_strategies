...
austraits <- read_rds("data/austraits-4.2.0.rds")
```
...
```{r}
resprouting_taxa <- resprouting$taxon_name %>% unique() # 10991 taxa
seeding_taxa <- post_fire_seeding$taxon_name %>% unique() # 3593 taxa

intersecting_taxa <- intersect(resprouting_taxa, seeding_taxa)
# 3450 taxa intersect between resprouting_capacity and post_fire_recruitment
```
...
# 4611 herbs, 6366 woody, 67 semi-woody, 47 unclear, 11 NA
```
