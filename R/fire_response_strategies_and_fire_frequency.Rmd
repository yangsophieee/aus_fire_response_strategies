...
glm_seeding <- glm(
  seeding_binomial ~ poly(log10(mean_fri), 2) * woody_or_herb,
  data = data.frame(training_x, training_y),
  family = binomial
)
summary(glm_seeding)
...
resprouting_plot + seeding_plot + resprouting_density + seeding_density + density_legend +
  plot_layout(design = design, heights = c(8, 3, 2.5, 3.5))
...
pixels) out of 10406 species with pixels >= 10. The warning was "ran out of iterations and did not converge". This occurred
...
In the Poisson regression method, out of 10387 species with pixels >= 10, 159 did not fit and they were all because all pixels were unburnt.
...
mean_fires_poisson %>%
  ...
  ggplot(aes(mean_fires, group = resprouting_binomial, fill = resprouting_binomial)) +
...
mean_fires_poisson %>%
  drop_na(resprouting_binomial) %>%
  ggplot(aes(mean_fires, fill = resprouting_binomial)) +
...
mean_fires_poisson %>%
  drop_na(resprouting_binomial) %>%
  filter(woody_or_herb %in% c("woody", "herb")) %>%
  ggplot(aes(mean_fires, ..count.., fill = resprouting_binomial)) +
  geom_density(adjust = 1.5, position = "fill", alpha = 0.6) +
  facet_wrap(~woody_or_herb) +
  scale_x_log10() +
  scale_fill_brewer(palette = "Set1") +
...
mean_fires_poisson %>%
  drop_na(seeding_binomial) %>%
  filter(woody_or_herb %in% c("woody", "herb")) %>%
  ggplot(aes(mean_fires, group = seeding_binomial, fill = seeding_binomial)) +
  geom_density(alpha = 0.6) +
  facet_wrap(~woody_or_herb) +
  scale_x_log10() +
  scale_fill_brewer(palette = "Set1") +
...
mean_fires_poisson %>%
  drop_na(seeding_binomial) %>%
  ggplot(aes(mean_fires, fill = seeding_binomial)) +
  geom_histogram(alpha = 0.6, position = "identity", binwidth = 0.1) +
  scale_x_log10() +
  scale_fill_brewer(palette = "Set1") +
  theme_classic()
...
mean_fires_poisson %>%
  drop_na(seeding_binomial) %>%
#  filter(mean_fires < 1100) %>% # Filter out high mean FRIs
  ggplot(aes(mean_fires, fill = seeding_binomial)) +
  geom_histogram(alpha = 0.6, position = "fill", binwidth = 0.1) +
  scale_x_log10() +
  scale_fill_brewer(palette = "Set1") +
...
mean_fires_poisson %>%
  drop_na(seeding_binomial) %>%
  filter(woody_or_herb %in% c("woody", "herb")) %>%
  ggplot(aes(mean_fires, ..count.., fill = seeding_binomial)) +
  geom_density(adjust = 1.5, position = "fill", alpha = 0.6) +
  facet_wrap(~woody_or_herb) +
  scale_x_log10() +
  scale_fill_brewer(palette = "Set1") +
