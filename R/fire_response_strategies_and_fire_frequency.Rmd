...
# Not ordered woody_or_herb factor
df_woody_herb_only <-
  mean_fires_clean %>%
  dplyr::filter(woody_or_herb %in% c("woody", "herb")) |>
  mutate(across(c("resprouting_binomial", "seeding_binomial", "woody_or_herb"), as.factor))

# Ordered woody_or_herb factor
df_woody_herb_only <-
  mean_fires_clean %>%
  dplyr::filter(woody_or_herb %in% c("woody", "herb")) |>
  mutate(across(c("resprouting_binomial", "seeding_binomial"), as.factor)) |>
  mutate(woody_or_herb = factor(woody_or_herb, ordered = TRUE, levels = c("woody", "herb")))
```
...
# Why does this create a different plot?
sorted_data <- sorted_data |>
  mutate(
    resprouting_binomial = as.factor(resprouting_binomial),
    woody_or_herb = factor(woody_or_herb, ordered = TRUE, levels = c("woody", "herb"))
  )
...
m2 <- glm(
  seeding_binomial ~
    log10(mean_fri) + I(log10(mean_fri)^2) + log10(mean_fri):woody_or_herb +
    I(log10(mean_fri)^2):woody_or_herb + woody_or_herb,
  data = df_woody_herb_only,
  family = binomial
)
summary(m2)
...
p3 <-
  ...
  scale_colour_manual(values = c("#7e5137"),
            name = "",
            breaks = c("woody"),
            labels = c("Woody")) +
  ggtitle("b)") +
