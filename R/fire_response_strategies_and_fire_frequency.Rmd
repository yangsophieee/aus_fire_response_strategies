...
mean_fires |> filter(!is.na(warnings) | !is.na(error) | !is.na(messages)) |> nrow() # 359
mean_fires |> filter(num_unburnt_pixels == num_pixels) |> nrow() # 298
mean_fires |> nrow() # 10871
mean_fires |> filter(num_pixels < 10) |> nrow()
```
...
# TODO rerun analyses
mean_fires_clean |> dplyr::filter(conf_int > 5) |> nrow() # 307
...
df_woody_herb_only |> arrange(-mean_fires_100) |> head(10) |> View()
```
