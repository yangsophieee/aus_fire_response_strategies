...
median_fris |> dplyr::filter(!is.na(med_w_warnings)) |> nrow()

median_fires_weibull <- median_fris %>%
  dplyr::filter(is.na(med_w_warnings)) %>%
  mutate(across(
...
With the survival analysis method, 1134 models did not converge (incl. unburnt
pixels) out of 10871 species. The warning was "ran out of iterations and did not converge". This occurred
...
In the Poisson regression method, out of 10871 species, 359 did not fit and they were all because all pixels were unburnt.

```{r}
mean_fires <- read_csv("outputs/mean_fires_df_with_poisson_glm.csv", guess_max = 10871)
mean_fires <- mean_fires %>% left_join(resprouting_seeding, by = c("taxon_name"))

mean_fires |> dplyr::filter(!is.na(warnings) | !is.na(messages) | !is.na(error)) |> nrow()
