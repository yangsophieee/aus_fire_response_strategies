...
```{r}
mean_fires_clean <- mean_fires_no_errors %>% filter(upr_confint > 0)
```
...
m1 <- glm(
  resprouting_binomial ~ log10(mean_fri) + I(log10(mean_fri)^2) + log10(mean_fri):woody_or_herb + I(log10(mean_fri)^2):woody_or_herb + woody_or_herb,
  data = df_woody_herb_only,
  family = binomial
)
...
mean_fires_clean %>% filter(upr_confint - lwr_confint < 100) %>% nrow()

y <- mean_fires_clean %>% mutate(confint_size = upr_confint - lwr_confint)
hist(y$confint_size)
```
